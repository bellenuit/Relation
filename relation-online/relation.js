...
function readFileSync(path, mode) {
fs.readFileSync(path, mode); }

function writeFileSync(path,s, mode) {
fs.writeFileSync(path, s, mode); }
...
function linehandler(line) {
  ...
  try {
  switch(command) {
  	case '//':	 console.log(body); swHistory.push(line); break;
  	case 'arity':	
 	...
 	case 'edit':
 		  ...
 		  if (pos < 1 || pos > swHistory.length) { console.log('Error: edit position out of range'); break; }
 		  var body2 = body.substr(pos.length+1).trim();
 		  if (body2 !='')
 		 	swHistory.splice(pos*1-1,rem,body2);
 		  else
 		  	swHistory.splice(pos*1-1,rem);
 	      break;
   	case 'extend':
   		  if (stack.length < 1) { console.log('Error: stack empty'); break; }
 		  rel = stack.pop(); try { 
 	...
 	case 'graph':
  	...
  	case 'help':
  	     ...
  	     {  			
 		  	...
 		  	case 'order': console.log('order column mode (, column mode)* : orders by columns. Modes are A (alphabetically), Z (alphabetically reversed), 1 (numerical low first), 9 (numerical high first)'); break;
  		  	...
  		  	default: console.log('Commands: arity count difference dup edit extend header help history insert join limit order p pop print project q quit read relation run swap union write'); 
  	...
  	case 'p':
  		 ...
  		 swHistory.push('print'); break;
   	case 'project':
   		  if (stack.length < 1) { console.log('Error: stack empty');  break; }
 		  rel = stack.pop(); try { rel=rel.project(commafields);  swHistory.push(line); } catch(e) { console.log(e.toString()) ; }  stack.push(rel); break;
    //case 'q': rl.close(); break;
  	case 'read': 
  		try
  		{
  			...
  			{
  				case 'txt':  s = readFileSync(body,'utf8'); rel = rel.fromTab(s); stack.push(rel); swHistory.push(line); break;
  				case 'csv':  s = readFileSync(body,'utf8'); rel = rel.fromCSV(s); stack.push(rel); swHistory.push(line); break;
  				case 'rel':  var list = readFileSync(body,'utf8').split('\n');
  							 interactive = 0;
  							 for (t in list) {
								linehandler(list[t]);
							}
  				...
  				case 'son': if (body.substr(-3) == 'json')
  							{
  								s = readFileSync(body,'utf8');
  								openProject(s);
   	...
   	case 'rename':
   		  if (stack.length < 1) { console.log('Error: stack empty'); break; }
 		  rel = stack.pop(); try { rel=rel.rename(commafields); swHistory.push(line);} catch(e) { console.log(e.toString()) ; } stack.push(rel); 
 		  break;
    case 'run':	var list = swHistory.slice(); swHistory = []; stack = [];
  							 interactive = 0;
  							 for (t in list) {
								linehandler(list[t]);
							}
   	...
   	case 'select':
   		  if (stack.length < 1) { console.log('Error: stack empty'); break; }
 		  rel = stack.pop(); try { rel=rel.select(body); swHistory.push(line);} catch(e) { console.log(e.toString()) ; } stack.push(rel); 
 		  break;
   	case 'swap':
   		  if (stack.length < 2) { console.log('Error: stack empty'); break; }
 		  rel2 = stack.pop(); 
 	...
 	case 'write': 
 		if (stack.length < 1) { console.log('Error: stack empty'); break; }
  		try
  		{
  			...
  			switch(body.substr(-3))
  			{
  				case 'txt':  writeFileSync(body, rel.toTab(), 'utf8'); break;
  				case 'csv':  writeFileSync(body, rel.toCSV(), 'utf8'); break;
  				case 'rel':  writeFileSync(body, swHistory.join('\n'), 'utf8'); break;
  				default : throw 'Error: invalid filetype';
  			}
  		}
  		catch(e) { console.log(e.toString()) ; }
