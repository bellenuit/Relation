  	...
  	case 'help':
  	     ...
  	     {  			
 		  	...
 		  	case 'order': console.log('order column mode (, column mode)* : orders by columns. Modes are A (alphabetically), Z (alphabetically reversed), 1 (numerical low first), 9 (numerical high first)'); break;
  		  	...
  		  	console.log('Possible aggregators: AVG, COUNT, MIN, MAX, MEDIAN, STDEV, SUM'); break;
   	...
   	case 'join':
 		  ...
 		  rel2 = stack.pop(); 
 		  rel = stack.pop(); 
 		  rel=rel.join(rel2,body.trim()); stack.push(rel); 
 		  history.push(line); break;
   	case 'limit':
   		  if (stack.length < 1) { console.log('Error: stack empty'); break; }
...
if (interactive)
{
	console.log('Relation 1.1 © 2019 belle-nuit.com');
...
function swExpression(expression, extentions)
{
	...
	arities['sqrt'] = 1; functions['sqrt'] = function(a) { if (a[0]<0) throw 'ERROR root on negative';  return Math.sqrt(a[0]) ; };
	arities['floor'] = 1; functions['floor'] = function(a) { return Math.floor(a[0]) ; };
	arities['abs'] = 1; functions['abs'] = function(a) { return Math.abs(a[0]) ; };
	arities['max'] = 2; functions['max'] = function(a,b) { return Math.max(a[0],a[1]) ; };
	arities['min'] = 2; functions['min'] = function(a,b) { return Math.min(a[0],a[1]) ; };
	...
	arities['bar'] = 1; functions['bar'] = function(a) { 
		var high = Math.floor(a[0] / 8) ;
		var low = Math.floor(a[0] - 8 * high);
		var car = ["","⡀","⡄","⡆","⡇","⣇","⣧","⣷","⣿"];
		var s = "⣿".repeat(high) + car[low];
		return s;	
	};
	
	arities['plusminus'] = 1; functions['plusminus'] = function(a) { 
		if (a[0] > 0) return "+".repeat(a[0]);
		if (a[0] < 0) return "-".repeat(-a[0]);
		return "";	
	};
	...
	this.evaluate = function(values)
	{
		...
		if (stack.length>1) throw 'ERROR unresolved terms '+this.rpn;
...
function swRelation(columns) {
	...
	this.join = function(rel, cond) {
		if (cond.toUpperCase() == "CROSS") return rel.join(this,"1 = 1");
		...
		try { var expr = new swExpression(cond); }
		catch (e) { throw "join compilation error: "+ cond+" "+e; }
		...
		for (var t in this.tuples){
			...
			//	console.log(parameters);
				...
				catch (e) { throw "join evaluation error: "+ cond+" "+e; }
